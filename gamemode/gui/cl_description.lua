...
function PANEL:Paint()
    ...
    if self.item then
        ...
        /*if self.item.entity_properties and self.item.entity_properties.is_arccw_attachment then
            local icon = nil
            local description = ""
            local atttbl = ustomizableWeaponry.registeredAttachmentsSKey[self.item.class]
            if atttbl.Trivia then
                description = self.Trivia.text
                
                if atttbl.Trivia.textFormatFunc then
                    description = atttbl.Trivia:textFormatFunc(atttbl)
                end
            end
            if atttbl.displayIcon then
        ...
        else*/if self.item.extra_description then
